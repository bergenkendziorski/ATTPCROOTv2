...
void run_digi_fiss(int runNum = 0)
{
   ...
   TString mcFile = inOutDir + TString::Format("symFission%02d.root", runNum);
   ...
   auto psa2 = make_unique<AtPSATBAvg>();
   psa2->SetThreshold(threshold);
   psa2->SetMaxThreshold(maxThreshold);
   //psa->SetMaxFinder();

   AtPSAtask *psaTask2 = new AtPSAtask(std::move(psa2));
   psaTask2->SetInputBranch("AtRawEvent");
   psaTask2->SetOutputBranch("AtEventRaw");
   psaTask2->SetPersistence(kFALSE);
...
auto method2 = std::make_unique<SampleConsensus::AtSampleConsensus>(
	  ...
	  SampleConsensus::Estimators::kChi2, AtPatterns::PatternType::kLine, RandomSample::SampleMethod::kUniform);
   method2->SetDistanceThreshold(7.5);
   method2->SetNumIterations(500);
   method2->SetMinHitsPattern(100);
   ...
   sacTask2->SetInputBranch("AtEventRaw");   
   sacTask2->SetOutputBranch("AtPatternEventLine");
   ...
   //fRun->AddTask(psaTask2);
   fRun->AddTask(scTask);
   fRun->AddTask(sacTask);
   //fRun->AddTask(sacTask2);
